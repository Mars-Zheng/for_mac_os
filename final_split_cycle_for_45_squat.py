...
def load_csv_data(file_path):
    try:
        df = pd.read_csv(file_path, skiprows=21, header=None)
        if not df.empty:
...
def out_put_csv(df_filtered, input_file_path):
    ...
    df_filtered.loc[:, "cos"] = np.cos(phase_angle)
...
def calculate_linear_regression(df, x1_column, x2_column, x3_column, y_column):
    """
    Perform multiple linear regression without an intercept and return a summary of the results.
    """
    try:
        # Extract the independent variables (X) and the dependent variable (y)
        X = df[[x1_column, x2_column, x3_column]]
        y = df[y_column]

        # Fit the model WITHOUT adding a constant (no intercept)
        model = sm.OLS(y, X).fit()

        # Store the predicted values in the DataFrame
        df[f'predicted_{y_column}'] = model.predict(X)

        # Collect the regression summary
        summary = {
            'independent_vars': [x1_column, x2_column, x3_column],
            'dependent_var': y_column,
        ...
        print(f"Linear Regression (without intercept) for {y_column}:")
        print(model.summary())
        return summary

    except Exception as e:
        messagebox.showerror("Error", f"Linear regression failed: {str(e)}")
        return None
    ...
    try:
        ...
        if df is not None:
            ...
            summary = calculate_linear_regression(
                df_filtered2, 'zp_std', 'zv_std', 'zp_std_cubic', 'za_std'
            )
